...
- 同步反馈：统一上传、下载和双向同步的进度与完成通知
- 进度显示：使用固定高度的单行状态卡片和圆形进度指示器，减少界面跳动
- 编辑体验：拆分待办编辑和同步通知组件，提升前端结构的可维护性
- 密码待办：抽离密码待办工具并补充单元测试，保持加密数据处理稳定

### 发布说明

- `v0.4.5` 是 LightTodo 的同步反馈与前端结构优化版本
- 本版本不改变数据库结构和 WebDAV 同步协议
...
- Android 首次同步：WebDAV 大批量待办先完整下载并校验，再统一导入本地，避免网络中断后每次启动只增加一部分数据
- 同步进度：下载和导入过程中显示当前数量、总数及百分比进度条
- 同步反馈：准备索引、批量下载、导入和完成阶段均提供明确状态
- 签名安全：本地 Android 签名备份目录加入 Git 忽略规则，避免私钥被误提交

### 发布说明

- `v0.4.3` 重点改善 Android 首次配置 WebDAV 后的大数据量下载体验
...
- Android 签名：使用长期 Release 证书签名 APK，支持正常安装和后续覆盖升级
- macOS 安装包：Apple Silicon 与 Intel 分别构建 DMG 和更新包，不再发布通用架构安装包
...
- 周期任务：支持每天、每周、每月重复规则
- 自动生成：完成周期任务后自动生成下一次待办，原任务保留在完成记录中
- 数据兼容：周期规则支持 SQLite 迁移、WebDAV 同步和本地备份恢复

### 修复与改进

- 应用图标：放大 Windows、macOS 及移动端图标主体，减少四周留白
- 月度周期：自动处理不同月份的天数差异，例如 1 月 31 日顺延至 2 月最后一天

### 发布说明

- `v0.4.0` 是 LightTodo 的周期任务版本
- 本版本在 `v0.3.3` 基础上新增周期规则和完成后自动生成下一次待办的能力
